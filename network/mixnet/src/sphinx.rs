...
use crate::onion::OnionEncryption;
...
impl From<crate::onion::OnionError> for SphinxError {
    fn from(err: crate::onion::OnionError) -> Self {
...
impl SphinxPacket {
    ...
    pub fn create(
        ...
        if payload.len() < packet_size {
            payload.resize(packet_size, 0u8);
        }
    ...
    fn test_too_many_hops() {
        ...
        for i in 0..17 {
            relay_path.push(RelayHop::new(
                alloc::format!("relay{}", i).into_bytes(),
                [i as u8; 32],
            ));
        }
