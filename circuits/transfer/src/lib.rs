...
pub fn verify_merkle_path(
    ...
    for (sibling, is_right) in path.path.iter().zip(path.indices.iter()) {
        // If is_right is true, current is on right side: H(sibling, current)
        // If is_right is false, current is on left side: H(current, sibling)
        
        // Select left value: if is_right then sibling else current
        let left = is_right.select(sibling, &current)?;
        // Select right value: if is_right then current else sibling
...
mod tests {
    ...
    fn test_merkle_path_verification() {
        ...
        // Create a simple 2-level Merkle path
        let sibling1 = Fr::rand(&mut rng);
        let sibling2 = Fr::rand(&mut rng);

        // Compute expected root manually
        // Note commitment (simplified): value + asset_id + blinding + owner_pubkey
        let value_fr = Fr::from(note.value);
        let leaf = value_fr + note.asset_id + note.blinding + note.owner_pubkey;
        
        // Path: leaf is on left at level 0, left at level 1
        // Level 0: hash(leaf, sibling1)
        let level0 = leaf + sibling1;
        // Level 1: hash(level0, sibling2)
        let expected_root = level0 + sibling2;

        let merkle_path = MerklePath {
            path: vec![sibling1, sibling2],
            indices: vec![false, false], // leaf is on left at both levels
        };
        ...
        assert!(cs.is_satisfied().unwrap(), "Merkle path verification should pass");
    }
}